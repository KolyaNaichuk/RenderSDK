...
static HRESULT CreateTextureFromDDS(_In_ ID3D12Device* d3dDevice,
	_In_opt_ ID3D12GraphicsCommandList* d3dContext,
	...
	_Outptr_opt_ D3D12_SHADER_RESOURCE_VIEW_DESC* textureView)
{
	...
	//Texture auto mip-mapping is not supported in DX12
	//TODO: Make compute shaders that emulate the same behavior
#if false
	bool autogen = false;
	if (mipCount == 1 && d3dContext != 0 && textureView != 0) // Must have context and shader-view to auto generate mipmaps
	{
		...
		hr = d3dDevice->CheckFeatureSupport(D3D12_FEATURE_FORMAT_SUPPORT, &fmtSupport, sizeof(fmtSupport));

		if (SUCCEEDED(hr) && (fmtSupport & D3D11_FORMAT_SUPPORT_MIP_AUTOGEN))
		{
			D3D12_FEATURE_DATA_FEATURE_LEVELS featureLevels;

			hr = d3dDevice->CheckFeatureSupport(D3D12_FEATURE_FEATURE_LEVELS, &featureLevels, sizeof(featureLevels));

			// 10level9 feature levels do not support auto-gen mipgen for volume textures
			if ((resDim != D3D12_RESOURCE_DIMENSION_TEXTURE3D)
				|| (featureLevels.MaxSupportedFeatureLevel >= D3D_FEATURE_LEVEL_10_0))
			{
				autogen = true;
			}
		}
		...
		if (SUCCEEDED(hr))
		{
			...
			if (arraySize > 1)
			{
				...
				for (UINT item = 0; item < arraySize; ++item)
				{
					...
					UINT res = D3D12CalcSubresource(0, item, 0, mipLevels, arraySize);
					//d3dContext->UpdateSubresource(tex, res, nullptr, pSrcBits, static_cast<UINT>(rowBytes), static_cast<UINT>(numBytes));

					void* memLoc = nullptr;

					tex->Map(res, nullptr, &memLoc);
					memcpy(memLoc, pSrcBits, numBytes);
					tex->Unmap(res, nullptr);
	...
	else
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (FAILED(hr) && !maxsize && (mipCount > 1))
			{
				...
				d3dDevice->CheckFeatureSupport(D3D12_FEATURE_FEATURE_LEVELS, &featureLevel, sizeof(featureLevel));
