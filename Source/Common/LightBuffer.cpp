...
LightBuffer::LightBuffer(D3DRenderEnv* pRenderEnv, u32 numPointLights, PointLight** ppPointLights)
	...
	, m_pLightViewProjMatrixBuffer(nullptr)
{
	...
	D3DStructuredBufferDesc lightBoundsBufferDesc(m_NumLights, sizeof(Sphere), true, false);
	D3DStructuredBufferDesc lightPropsBufferDesc(m_NumLights, sizeof(PointLightProps), true, false);

	m_pLightBoundsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightBoundsBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightBoundsBuffer");
	m_pLightPropsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightPropsBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightPropsBuffer");

	m_pUploadLightBoundsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pUploadHeapProps, &lightBoundsBufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, L"LightBuffer::m_pUploadLightBoundsBuffer");
	m_pUploadLightBoundsBuffer->Write(lightBounds.data(), m_NumLights * sizeof(Sphere));

	m_pUploadLightPropsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pUploadHeapProps, &lightPropsBufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, L"LightBuffer::m_pUploadLightPropsBuffer");
	m_pUploadLightPropsBuffer->Write(lightProps.data(), m_NumLights * sizeof(PointLightProps));
}

LightBuffer::LightBuffer(D3DRenderEnv* pRenderEnv, u32 numSpotLights, SpotLight** ppSpotLights)
	...
	, m_pLightViewProjMatrixBuffer(nullptr)
{
	...
	D3DStructuredBufferDesc lightBoundsBufferDesc(m_NumLights, sizeof(Sphere), true, false);
	D3DStructuredBufferDesc lightPropsBufferDesc(m_NumLights, sizeof(SpotLightProps), true, false);
	D3DStructuredBufferDesc lightFrustumBufferDesc(m_NumLights, sizeof(LightFrustum), true, false);
	D3DStructuredBufferDesc lightViewProjMatrixBufferDesc(m_NumLights, sizeof(Matrix4f), true, false);

	m_pLightBoundsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightBoundsBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightBoundsBuffer");
	m_pLightPropsBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightPropsBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightPropsBuffer");
	m_pLightFrustumBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightFrustumBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightFrustumBuffer");
	m_pLightViewProjMatrixBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pDefaultHeapProps, &lightViewProjMatrixBufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, L"LightBuffer::m_pLightViewProjMatrixBuffer");
	...
	m_pUploadLightFrustumBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pUploadHeapProps, &lightFrustumBufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, L"LightBuffer::m_pUploadLightFrustumBuffer");
	m_pUploadLightFrustumBuffer->Write(lightFrustums.data(), m_NumLights * sizeof(LightFrustum));

	m_pUploadLightViewProjMatrixBuffer = new D3DBuffer(pRenderEnv, pRenderEnv->m_pUploadHeapProps, &lightViewProjMatrixBufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, L"LightBuffer::m_pUploadLightViewProjMatrixBuffer");
...
void LightBuffer::RecordDataForUpload(D3DCommandList* pCommandList)
{
	...
	{
		D3DResourceTransitionBarrier(m_pLightBoundsBuffer, m_pLightBoundsBuffer->GetState(), m_pLightBoundsBuffer->GetReadState()),
		D3DResourceTransitionBarrier(m_pLightPropsBuffer, m_pLightPropsBuffer->GetState(), m_pLightPropsBuffer->GetReadState()),
		D3DResourceTransitionBarrier(m_pLightFrustumBuffer, m_pLightFrustumBuffer->GetState(), m_pLightFrustumBuffer->GetReadState()),
		D3DResourceTransitionBarrier(m_pLightViewProjMatrixBuffer, m_pLightViewProjMatrixBuffer->GetState(), m_pLightViewProjMatrixBuffer->GetReadState())
	};
